...
object cpkMesProcessMonth {
  ...
  private def CpkCalculate(sparkSession: SparkSession, sc: SparkContext, kuduMaster: String, tableName: String,
    ...
    sparkSession.sql("SELECT * FROM temp1")
      .where(s"MDATE>'${createTime}' and MDATE<='${endtime(createTime)}' and FIRST_TIME='巡检'")
//            .where(s"MDATE >= '2021-10-01 00:00:00' and MDATE <= '2021-10-31 00:00:00' and FIRST_TIME='巡检'")
//      .where(s"MDATE >= '2021-07-01 00:00:00' and MDATE <= '2021-09-30 00:00:00' and FIRST_TIME='巡检'")
          ...
          if (usl != "null" && lsl != "null" && lsl != "" && usl != "") {
            ...
            if (uslavgdata > lslavgdata) {
              ...
              if (cpu.toFloat >= cpl.toFloat) {
                ...
                insertData(kuduMaster, insertTable, id, dateYear(mdate), Year(mdate), s"${yearMonth(mdate)}", s"${firstTime}", s"${workShopCode}",
  ...
  def startTime(date: String): String = {
    ...
    if (date.isEmpty || date.contains("NULL") || date.contains("null")) {
      ...
      val strDate: String = date.substring(6,16)
      val newtime: Date = new SimpleDateFormat("yyyy-MM-dd").parse(strDate)
      var cal = Calendar.getInstance()
      cal.setTime(newtime)
      cal.add(Calendar.MONTH, 1)
