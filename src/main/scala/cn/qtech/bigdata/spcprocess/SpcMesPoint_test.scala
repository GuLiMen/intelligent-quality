...
//      .where(s"mdate>'2021-11-01 00:00:00' and mdate<='2021-11-03 00:00:00' and workshop_code = '古二COB二区'")
        ...
        if (tdataDatas.size == 1 && groupDataOnePcsList.size >= 2 && maxcreatedtime > timemin) {
          ...
          if (otherTdatadatas.nonEmpty) {
            if (avgdata >= otherTdatadatas.max) {
              ...
              result += id +","+ firstTimeField+","+workShopCodeField+","+partSpecField+","+ opCodeField+","+
              staCodeField+","+eattribute1_1Field+","+ testItemField+","+ factoryField+","+ id_NoField+","+ rCardField+","+ maxcreatedtime+","+
              avgdata.formatted("%.3f").toString+","+ rangeonepcs.toString+","+ "null"
...
//                avgdata.formatted("%.3f").toString, rangeonepcs.toString, "")
          ...
          result +=id+","+ firstTimeField+","+ workShopCodeField+","+ partSpecField+","+ opCodeField+","+
            staCodeField+","+ eattribute1_1Field+","+ testItemField+","+ factoryField+","+ id_NoField+","+ rCardField+","+ maxcreatedtime+","+
            avgdata.toString+","+ rangedata.toString+","+ "null"
//          insertData(kuduMaster, insertTable, id, firstTimeField, workShopCodeField, partSpecField, opCodeField,
//            staCodeField, eattribute1_1Field, testItemField, factoryField, id_NoField, rCardField, maxcreatedtime,
//            avgdata.toString, rangedata.toString, "")
          ...
          result +=id+","+ firstTimeField+","+ workShopCodeField+","+ partSpecField+","+ opCodeField+","+
            staCodeField+","+ eattribute1_1Field+","+ testItemField+","+ factoryField+","+ id_NoField+","+ rCardField+","+ maxcreatedtime+","+
            avgdata.toString+","+ "null"+","+ stddevdata.toString
//          insertData(kuduMaster, insertTable, id, firstTimeField, workShopCodeField, partSpecField, opCodeField,
//            staCodeField, eattribute1_1Field, testItemField, factoryField, id_NoField, rCardField, maxcreatedtime,
//            avgdata.toString, "", stddevdata.toString)
    ...
    val resultRDD: RDD[String] = sc.makeRDD(result)

    val value: RDD[(String, String, String, String, String, String, String, String, String, String, String, String, String, String, String)] = resultRDD.map(res => {
      val datas: Array[String] = res.split(",")
      (datas(0), datas(1), datas(2), datas(3), datas(4), datas(5), datas(6), datas(7), datas(8), datas(9), datas(10), datas(11), datas(12), datas(13), datas(14))
    })
    value.toDF().createOrReplaceTempView("resultDF")

    sparkSession.sql("insert into pointtable select * from resultDF")
  }
