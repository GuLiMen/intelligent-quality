...
object SpcMesControlLimit {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spcclresultlist = "ADS_SPCMESCONTROLLIMIT_TEST"
  ...
  SparkContext, kuduMaster: String, tableName: String, spcclresultlist: String): Unit = {
    ...
    var timemin = "2020-06-01 01:12:51.303"
...
//      .where(s"mdate>'${timemin}' and mdate<='${sourcetimemax}'")
      .where(s"mdate>'2020-06-01 01:12:51' and mdate<='2021-10-01 00:00:00'")
    ...
    while (datalist.hasNext) {
      ...
      if (integer.get() == numField.toInt) {
        ...
        if (idNoList.distinct.size >= 30) {
          ...
          val staCode = groupDatasFirstArr {
            10
          };
          ...
          if (sumAllIdNoList.nonEmpty && sumAllIdNoList.length >= 30) {
            ...
            if (pcsByTdataPcsCount == 1) {
              ...
              if (upper != "null" || lower != "null") {
                ...
                result += firstTime+","+ workShopCode+","+
                  partSpec+","+ opCode+","+ staCode+","+
                  eattribute1_1+","+ testItem+","+ factory+","+ spcruntime+","+
                  pcsByTdataPcsCount+","+ upper+","+
                  lower+","+ value1Ucl+","+ value1Lcl+","+ range1Ucl+","+ range1Lcl+","+ ""+","+ ""

//                upsertData(kuduMaster, spcclresultlist, s"${firstTime}", s"${workShopCode}",
//                  s"${partSpec}", s"${opCode}", s"${staCode}",
//                  s"${eattribute1_1}", s"${testItem}", s"${factory}", spcruntime,
//                  pcsByTdataPcsCount, s"${upper}",
//                  s"${lower}", value1Ucl, value1Lcl, range1Ucl, range1Lcl, "", "")
              ...
              if (upper != "null" || lower != "null") {
                ...
                result+=firstTime+","+ workShopCode+","+
                  ...
                  eattribute1_1+","+ testItem+","+ factory+","+ spcruntime+","+
                  pcsByTdataPcsCount+","+ upper+","+
                  lower+","+ value2to5Ucl+","+ value2to5Lcl+","+ range2to5Ucl+","+ range2to5Lcl+","+ ""+","+ ""
...
//                  s"${lower}", value2to5Ucl, value2to5Lcl, range2to5Ucl, range2to5Lcl, "", "")
              ...
              if (upper != "null" || lower != "null") {
                ...
                result += firstTime+","+ workShopCode+","+
                  ...
                  eattribute1_1+","+ testItem+","+ factory+","+ spcruntime+","+
                  pcsByTdataPcsCount+","+ upper+","+
                  lower+","+ value6Ucl+","+ value6Lcl+","+ ""+","+ ""+","+ stddev6Ucl+","+ stddev6Lcl
...
//                  s"${lower}", value6Ucl, value6Lcl, "", "", stddev6Ucl, stddev6Lcl)
    ...
    for(datas <- result){
      ...
      rowUpsert.addString("FIRST_TIME", data(0))
      rowUpsert.addString("WORKSHOP_CODE", data(1))
      rowUpsert.addString("PART_SPEC", data(2))
      rowUpsert.addString("OP_CODE", data(3))
      rowUpsert.addString("STA_CODE", data(4))
      rowUpsert.addString("EATTRIBUTE1_1", data(5))
      rowUpsert.addString("TESTITEM", data(6))
      rowUpsert.addString("FACTORY", data(7))
      rowUpsert.addString("TIME", data(8))
      rowUpsert.addInt("PCS", data(9).toInt)
      rowUpsert.addString("USL", data(10))
      rowUpsert.addString("LSL", data(11))
      rowUpsert.addString("AVGUCL", data(12))
      rowUpsert.addString("AVGLCL", data(13))
      rowUpsert.addString("RANGEUCL", data(14))
      rowUpsert.addString("RANGELCL", data(15))
      rowUpsert.addString("STDDEVUCL", data(16))
      rowUpsert.addString("STDDEVLCL", data(17))
