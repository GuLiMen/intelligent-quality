...
object SpcPointForHuaWei {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val originTable = "ADS_DATAJOINHUAWEIWB"
    val spccontrollimit = "ADS_SPCCONTROLLIMITFORHUAWEI"
    val insertTable = "ADS_SPCHUAWEIPOINTS"

    PointCalculate(sparkSession: SparkSession, sc:
      SparkContext, kuduMaster: String, originTable: String, spccontrollimit: String,insertTable:String)

    sc.stop()
  }

  private def PointCalculate (sparkSession: SparkSession, sc:
  SparkContext, kuduMaster: String, originTable: String, spccontrollimit: String, insertTable:String): Unit = {
    val optionorigin = Map(
      "kudu.master" -> kuduMaster,
      "kudu.table" -> originTable
    )
    ...
    sparkSession.read.options(optionorigin).format("kudu").load
      .createOrReplaceTempView("originnofiltertime")
    sparkSession.read.options(optionsclresult).format("kudu").load
      .createOrReplaceTempView("controllimit")
    ...
    var sourcetimelists = sparkSession.sql("select min(mdate),max(mdate) from originnofiltertime")
      .collect().toList
    ...
    sparkSession.sql("select * from originnofiltertime")
      .where(s"mdate>'${minTime}' and mdate<='${sourcetimemax}'")
      .createOrReplaceTempView("origin")
    ...
    while (datalist.hasNext) {
      ...
      if (integer.get() == numfield.toInt) {
        ...
        val id_NoField = groupDatasArr {3}.toDouble;
  ...
  def insertData(kuduMaster: String, inserttable: String, id:String, firsttime:String, workshopcode: String,
                 partspec: String, opcode: String, stacode: String, eattribute11:String, testitem: String,
                 factory:String, idno:Double, rcard:String, mdate: String, avgvalue :String, rangevalue: String,
